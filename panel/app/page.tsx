...
type StartPreflightItemStatus = "ok" | "warn" | "danger" | "unknown";

type StartPreflightData = {
  inst: string;
  jar: string;
  port: number;
  javaPath: string;
...
export default function HomePage() {
  ...
  // Start preflight (Games)
  const [startPreflightOpen, setStartPreflightOpen] = useState<boolean>(false);
  const [startPreflightBusy, setStartPreflightBusy] = useState<boolean>(false);
  const [startPreflightStatus, setStartPreflightStatus] = useState<string>("");
  const [startPreflightData, setStartPreflightData] = useState<StartPreflightData | null>(null);
  const startPreflightResolveRef = useRef<((ok: boolean) => void) | null>(null);
  const startPreflightRunRef = useRef<number>(0);
  ...
  async function ensureEulaAccepted(instance: string) {
    const inst = String(instance || "").trim();
    ...
    try {
      ...
      baseText = b64DecodeUtf8(String(out?.b64 || ""));
    } catch {
      baseText = "";
    }

    const cur = baseText ? String(getPropValue(baseText, "eula") || "").trim().toLowerCase() : "";
    if (cur === "true") return { already: true, path: eulaPath };
    ...
    await callOkCommand("fs_write", { path: eulaPath, b64: b64EncodeUtf8(nextText) }, 10_000);
    return { already: false, path: eulaPath };
  }
  ...
  function closeStartPreflight(ok: boolean) {
    setStartPreflightOpen(false);
    setStartPreflightBusy(false);
    setStartPreflightStatus("");
    const resolve = startPreflightResolveRef.current;
    startPreflightResolveRef.current = null;
    if (resolve) resolve(ok);
  }

  function badgeClassForPreflightStatus(s: StartPreflightItemStatus) {
  ...
  async function collectStartPreflight(inst: string, jar: string, port: number, javaPath: string): Promise<StartPreflightData> {
    ...
    try {
      if (Number.isFinite(port) && port > 0) {
        const out = await callOkCommand("net_check_port", { port }, 10_000);
        if (out?.available === false) {
          portAvailable = false;
          portError = String(out?.error || "in use");
        } else {
          portAvailable = true;
        }
      }
    ...
    const eulaPath = joinRelPath(inst, "eula.txt");
    let eulaAccepted: boolean | null = null;
    let eulaError = "";
    try {
      const out = await callOkCommand("fs_read", { path: eulaPath }, 10_000);
      ...
      eulaAccepted = v === "true";
      if (!eulaAccepted) eulaError = "eula!=true";
    } catch (e: any) {
      eulaAccepted = false;
      eulaError = String(e?.message || e);
    }
    ...
    return new Promise<boolean>((resolve) => {
      ...
      (async () => {
        try {
          ...
          if (startPreflightRunRef.current !== runId) return;
          setStartPreflightStatus(String(e?.message || e));
        } finally {
          if (startPreflightRunRef.current !== runId) return;
          setStartPreflightBusy(false);
        }
  ...
  async function fsHashEntry(pathRaw: string, opts: { maxBytes?: number } = {}, timeoutMs = 60_000) {
    const p = String(pathRaw || "")
      .replace(/\\+/g, "/")
      .replace(/\/+/g, "/")
      .replace(/^\/+/, "")
    ...
    const maxBytes = typeof opts.maxBytes === "number" && opts.maxBytes > 0 ? Math.floor(opts.maxBytes) : undefined;
      ...
      >
        <ModalStackProvider>
          ...
          <ManagedModal
            ...
            onOverlayClick={() => closeStartPreflight(false)}
            modalStyle={{ width: "min(640px, 100%)" }}
            ariaLabel={t.tr("Preflight", "预检")}
          >
            <div className="modalHeader">
              <div>
                <div style={{ fontWeight: 800 }}>{t.tr("Preflight", "预检")}</div>
                <div className="hint">
                  {t.tr("Review common start blockers before starting.", "启动前检查常见阻塞项。")}
                </div>
              </div>
              <button type="button" onClick={() => closeStartPreflight(false)}>
                {t.tr("Close", "关闭")}
              </button>
            </div>

            <div className="modalBody">
              {startPreflightStatus ? <div className="hint">{startPreflightStatus}</div> : null}

              <div className="grid2" style={{ gridTemplateColumns: "1fr" }}>
                {(() => {
                  const d = startPreflightData;
                  const inst = String(d?.inst || "").trim();
                  ...
                  const requiredJavaMajor = Math.round(Number(d?.requiredJavaMajor || 0));
                  const requiredJavaError = String(d?.requiredJavaError || "");

                  const lowDisk = diskFreeBytes > 0 && diskFreeBytes < 2 * 1024 * 1024 * 1024;
                  const diskStatus: StartPreflightItemStatus = diskFreeBytes <= 0 ? "unknown" : lowDisk ? "warn" : "ok";
                  const portStatus: StartPreflightItemStatus = d?.portAvailable == null ? "unknown" : d.portAvailable ? "ok" : "danger";
                  const javaStatus: StartPreflightItemStatus = requiredJavaError ? "unknown" : requiredJavaMajor > 0 ? "ok" : "unknown";
                  const eulaStatus: StartPreflightItemStatus = d?.eulaAccepted == null ? "unknown" : d.eulaAccepted ? "ok" : "danger";

                  const canStartNormally = !!d && d.eulaAccepted !== false;
                  const shouldWarnStart = !!d && (lowDisk || d.portAvailable === false);
                  ...
                  return (
                    <>
                      ...
                      <div className="field" style={{ gridColumn: "1 / -1" }}>
                        <label>{t.tr("Checklist", "检查项")}</label>
                        <div className="row" style={{ alignItems: "flex-start", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
                          <div style={{ minWidth: 0 }}>
                            <div className="row" style={{ gap: 8, alignItems: "center" }}>
                              <span className={badgeClassForPreflightStatus(eulaStatus)}>{t.tr("EULA", "EULA")}</span>
                              <span className="muted">
                                {d?.eulaAccepted === true
                                  ? t.tr("accepted", "已接受")
                                  : d?.eulaAccepted === false
                                    ? t.tr("not accepted", "未接受")
                                    : t.tr("unknown", "未知")}
                              </span>
                            </div>
                            {d?.eulaAccepted === false ? (
                              <div className="hint" style={{ marginTop: 4 }}>
                                {t.tr("Minecraft will refuse to start until eula=true.", "Minecraft 会拒绝启动，直到 eula=true。")}
                                {d?.eulaError ? ` (${String(d.eulaError)})` : ""}
                              </div>
                            ) : null}
                          </div>
                          <div className="btnGroup">
                            <button
                              ...
                              disabled={startPreflightBusy || !inst}
                            >
                            ...
                            <button
                              ...
                              onClick={async () => {
                                if (!inst) return;
                                await openFileByPath(joinRelPath(inst, "eula.txt"));
                              }}
                              disabled={startPreflightBusy || !inst}
                            >
                              {t.tr("Open eula.txt", "打开 eula.txt")}
                            </button>
                          </div>
                        </div>

                        <div className="row" style={{ alignItems: "flex-start", justifyContent: "space-between", gap: 10, flexWrap: "wrap", marginTop: 10 }}>
                          <div style={{ minWidth: 0 }}>
                            <div className="row" style={{ gap: 8, alignItems: "center" }}>
                              <span className={badgeClassForPreflightStatus(portStatus)}>{t.tr("Port", "端口")}</span>
                              <span className="muted">
                                {d?.portAvailable === true
                                  ? t.tr("available", "可用")
                                  : d?.portAvailable === false
                                    ? t.tr("in use", "可能被占用")
                                    : t.tr("unknown", "未知")}
                            ...
                            {d?.portAvailable === false ? (
                              <div className="hint" style={{ marginTop: 4 }}>
                                {t.tr("Starting may fail if the port is occupied.", "如果端口被占用，启动可能失败。")}
                                {d?.portError ? ` (${String(d.portError)})` : ""}
                              </div>
                            ) : null}
                          </div>
                          <div className="btnGroup">
                            <button
                              type="button"
                              onClick={() => {
                                closeStartPreflight(false);
                                openSettingsModal();
                              }}
                              disabled={startPreflightBusy || !inst}
                            >
                              {t.tr("Open Settings", "打开设置")}
                            </button>
                          </div>
                        </div>
                        ...
                        <div className="row" style={{ alignItems: "flex-start", justifyContent: "space-between", gap: 10, flexWrap: "wrap", marginTop: 10 }}>
                          <div style={{ minWidth: 0 }}>
                            <div className="row" style={{ gap: 8, alignItems: "center" }}>
                              <span className={badgeClassForPreflightStatus(javaStatus)}>{t.tr("Java", "Java")}</span>
                              <span className="muted">
                                {requiredJavaMajor > 0
                                  ? t.tr(`required >=${requiredJavaMajor}`, `需要 >=${requiredJavaMajor}`)
                                  : t.tr("unknown", "未知")}
                              </span>
                            </div>
                            {requiredJavaError ? <div className="hint" style={{ marginTop: 4 }}>{requiredJavaError}</div> : null}
                            <div className="hint" style={{ marginTop: 4 }}>
                              {javaPath
                                ? t.tr("You configured a fixed Java path.", "你配置了固定 Java 路径。")
                                : t.tr("Daemon will auto-pick a compatible Java if available.", "Daemon 会自动选择可用的兼容 Java（若存在）。")}
                            </div>
                          </div>
                          <div className="btnGroup">
                            <button
                        ...
                        <div className="row" style={{ alignItems: "flex-start", justifyContent: "space-between", gap: 10, flexWrap: "wrap", marginTop: 10 }}>
                          <div style={{ minWidth: 0 }}>
                            <div className="row" style={{ gap: 8, alignItems: "center" }}>
                              <span className={badgeClassForPreflightStatus(diskStatus)}>{t.tr("Disk", "磁盘")}</span>
                              <span className="muted">
                                {diskFreeBytes > 0
                                  ? `${t.tr("free", "可用")}: ${fmtBytes(diskFreeBytes)}${diskTotalBytes > 0 ? ` / ${fmtBytes(diskTotalBytes)}` : ""}`
                                  : t.tr("unknown", "未知")}
                              </span>
                            </div>
                            {lowDisk ? <div className="hint" style={{ marginTop: 4 }}>{t.tr("Low disk space may break installs/backups.", "磁盘空间偏低可能导致安装/备份失败。")}</div> : null}
                          </div>
                          <div className="btnGroup">
                            <button
                              type="button"
                              className="iconBtn iconOnly"
                              title={t.tr("Recheck", "重新检查")}
                              aria-label={t.tr("Recheck", "重新检查")}
                              onClick={async () => {
                                if (!inst) return;
                                setStartPreflightBusy(true);
                                setStartPreflightStatus(t.tr("Checking...", "检查中..."));
                        ...
                        {d?.checkedAtMs ? <div className="hint mt-2">{t.tr("Checked", "检查时间")}: {new Date(d.checkedAtMs).toLocaleString()}</div> : null}

                        {shouldWarnStart ? (
                          <div className="hint mt-2" style={{ color: "var(--danger)" }}>
                            {t.tr("Some checks look risky. You can still start, but it may fail.", "部分检查项存在风险。你仍可启动，但可能失败。")}
                          </div>
                        ) : null}

                        {!canStartNormally ? (
                          <div className="hint mt-2" style={{ color: "var(--danger)" }}>
                            {t.tr("EULA is not accepted. Accept it to avoid guaranteed start failure.", "EULA 未接受。建议先接受，否则会必然启动失败。")}
                          </div>
                        ) : null}
                      </div>
                    </>
            ...
            <div className="modalFooter">
              <button type="button" onClick={() => closeStartPreflight(false)} disabled={startPreflightBusy}>
                {t.tr("Cancel", "取消")}
              </button>
              <button
                type="button"
                ...
                disabled={startPreflightBusy || !startPreflightData?.inst}
              >
                {t.tr("Settings", "设置")}
              </button>
              <button
                ...
                className={(() => {
                  const d = startPreflightData;
                  const lowDisk = !!d && d.diskFreeBytes > 0 && d.diskFreeBytes < 2 * 1024 * 1024 * 1024;
                  const risky = !!d && (lowDisk || d.portAvailable === false || d.eulaAccepted === false);
                  return risky ? "dangerBtn" : "primary";
                })()}
                onClick={async () => {
                  const d = startPreflightData;
                  if (!d?.inst) return;

                  // If EULA isn't accepted, make the user explicitly confirm the bypass.
                  if (d.eulaAccepted === false) {
                    const ok = await confirmDialog(
                      t.tr(
                        "EULA is not accepted. Minecraft will refuse to start. Start anyway?",
                        "EULA 未接受。Minecraft 会拒绝启动。仍要启动吗？"
                      ),
                      { title: t.tr("Start anyway", "仍要启动"), confirmLabel: t.tr("Start anyway", "仍要启动"), cancelLabel: t.tr("Cancel", "取消"), danger: true }
                    );
                    if (!ok) return;
                  }
                  closeStartPreflight(true);
                }}
                disabled={startPreflightBusy}
              >
                {(() => {
                  const d = startPreflightData;
                  const lowDisk = !!d && d.diskFreeBytes > 0 && d.diskFreeBytes < 2 * 1024 * 1024 * 1024;
                  const risky = !!d && (lowDisk || d.portAvailable === false || d.eulaAccepted === false);
                  return risky ? t.tr("Start anyway", "仍要启动") : t.tr("Start", "启动");
                })()}
              </button>
            </div>
          </ManagedModal>
