...
pub fn validate_vanilla_params(params: &BTreeMap<String, String>) -> anyhow::Result<VanillaParams> {
    ...
    let name = params
        ...
        .map(|v| v.trim())
        .filter(|v| !v.is_empty())
        .unwrap_or("Alloy Sons of the Forest server")
        .to_string();

    let port = parse_port(params.get("port"), 8766, &mut field_errors, "port");
        ...
        Some(raw) => match raw.parse::<u32>() {
            ...
            Err(_) => {
                field_errors.insert(
                    ...
                    "Must be an integer between 1 and 16.".to_string(),
                );
                8
            }
        },
    ...
    if !field_errors.is_empty() {
        return Err(crate::error_payload::anyhow(
            "invalid_param",
            "invalid sons_of_the_forest params",
            Some(field_errors),
